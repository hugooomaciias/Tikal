...
export const StageProvider = ({ children }) => {
    ...
    const create = async (stageData) => {
        ...
        const data = await apiCall("/api/stage", "POST", stageData, {
    ...
    const update = async (id, stageData) => {
        const token = localStorage.getItem("accessToken");

        const data = await apiCall(`/api/stage/${id}`, "PATCH", stageData, {
            Authorization: `Bearer ${token}`,
        });
    ...
    const remove = async (id) => {
        ...
        const data = await apiCall(`/api/stage/${id}`, "DELETE", null, {
            Authorization: `Bearer ${token}`,
        });
