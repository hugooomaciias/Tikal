...
export const StagesCardComponent = () => {
     ...
     */
    const stagesOptions = [
        { colour: "bg-tertiary-200", title: "Base de datos", note: "Esta es una nota aclarativa sobre el  proyecto ‘Universidad’, en la que se  explican diversos aspectos de dicho proyecto" },
        ...
        <div className="h-full w-1/3 flex flex-col items-end justify-between p-6 bg-primary rounded-[2.5rem]">
            ...
            <div className="h-full w-full flex flex-col items-center gap-4">
                ...
                <div className="h-10 w-full flex items-center justify-between text-quaternary-700">
                    {! isStageSearchOpen && (
                        <span className="text-2xl font-bold">Fases</span>
                    )}

                    <div className={`flex items-center justify-end transition-all duration-500 ease-in-out rounded-full ${isStageSearchOpen ? 'w-full bg-primary-50 px-3 py-1.5 shadow-inner' : 'w-fit bg-transparent p-0'}`}>
                        
                        {/* Search Input (Expands when open) */}
                        <input type="text" placeholder="Buscar fase..." value={stageSearchQuery}
                            onChange={(e) => setStageSearchQuery(e.target.value)} autoFocus={isStageSearchOpen}
                ...
                <div className="h-fit w-full flex flex-col gap-3">
                    {stagesOptions.map((option, index) => {
                        ...
                        return (
                            <div key={index} onClick={() => setActiveStage(option.title)} onDoubleClick={() => setStageToEdit(option)}
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4">
                                    <div className={`h-6 w-6 ${isActive ? 'bg-primary' : option.colour} p-3 rounded-full`}></div>
            ...
            {stageToEdit && (
                <StagePopUpComponent onClose={() => setStageToEdit(null)} initialData={stageToEdit === "new" ? null : stageToEdit} />
            )}
        </div>
    )
}