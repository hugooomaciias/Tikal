...
export const TempleModePage = () => {
     ...
     *
     * Extracts all required business logic, including derived progression datasets,
     * UI interaction states, localization functions, and action handlers from the headless hook.
     */
    const { t, templeModeStates, templeModeData, templeModeActions } = useTempleModeLogic();

    const { isDataLoaded, isTotemsMenuOpen } = templeModeStates;
    const { data, additionalData } = templeModeData;
    const { toggleTotemsMenu } = templeModeActions;
            ...
            <section className="flex-1 flex flex-col gap-4 md:gap-6 w-full h-full overflow-hidden">
                ...
                <div className="flex flex-col gap-3 md:gap-4">
                    ...
                    <div className="flex items-center justify-between gap-4 w-full">
                        {/* Left Panel: Current Rank Badge & Global Progress */}
                        <div className="w-full md:w-fit min-w-[33%]">
                            <div className={`flex items-center gap-5 ${theme.wrapper} border shadow-lg rounded-[2rem] p-4 transition-all duration-300 backdrop-blur-sm`}>
                                ...
                                {/* Rank Metadata & Progress Track Container */}
                                <div className="flex-1 flex flex-col justify-between h-20 py-1.5 pr-2"> 
                                    {/* Titles & Statistics Row */}
                                    ...
                                    <div className="w-full flex items-center gap-2 mt-1">
                                        <div className="relative flex-1 group/progress cursor-pointer">
                                            ...
                                            {/* Hover Tooltip (Hours spent vs required) */}
                                            <div 
                                        ...
                                        <span className={`text-xs font-mono font-bold ${theme.subtitle}`}>
                                            {progressPercentage}%
                                        </span>
                                    </div>
                                </div>
                        ...
                        {/* Right Panel: Active Totem Target & Expandable Menu */}
                        <div className="relative w-full xl:w-fit min-w-[35%]">
                            ...
                            <div 
                                ...
                                `}
                            >
                                {/* Target Totem Info & Mini Preview */}
                                <div className="flex-1 flex items-center justify-between h-20 py-1.5 pr-2 gap-4">
                                    <div className="flex-1 flex flex-col justify-between h-full">
                                        <div className="flex flex-col">
                                            <div className="flex items-center gap-1.5 mb-0.5">
                                                ...
                                                <IconChevronDown 
                                                    stroke={2.5}
                                                    className={`w-3.5 h-3.5 opacity-70 transition-transform duration-300 ${theme.subtitle} ${isTotemsMenuOpen ? "rotate-180" : "group-hover:translate-y-0.5"}`} 
                                                />
                                            </div>
                                            <h3 className={`font-passero font-bold text-xl leading-none mt-1 truncate ${theme.title}`}>
                                        ...
                                        {/* Dynamic Target Totem Progress Bar */}
                                        {nextTargetTotem && (
                                            <div className="w-full flex items-center gap-2 mt-1">
                                                <div className="relative flex-1 group/progress cursor-pointer">
                                                    <div className={`h-2 rounded-full overflow-hidden ${theme.track}`}>
                            ...
                            {/* Floating Dropdown Menu: All Totems List */}
                                    ...
                                    {totems.map((totem) => {
                                        return (
                                            ...
                                            >
                                                ...
                                                {/* Totem Metadata & Progress */}
                                                <div className="flex-1 flex flex-col justify-center gap-1.5">
                                                    <div className="flex justify-between items-end">
                                                        <h4 className={`font-passero font-bold text-lg leading-none ${theme.title}`}>
                                                            {totem.name}
                                                        </h4>
                                                    ...
                                                    <div className="flex justify-between items-end">
                                                        <p className={`text-[11px] leading-tight ${theme.subtitle}`}>
                                                            <ScrollingText className={`text-[11px] leading-tight whitespace-nowrap ${theme.subtitle}`} text={totem.goalDescription} />
                                                        </p>

                                                        <span className={`text-[11px] font-mono font-bold leading-none ${theme.subtitle}`}>
