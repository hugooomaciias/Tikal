...
public class TimeLogService {
    ...
    public TimeLogDTO pause(TimeLogPause request, Integer id) {
        TimeLog timeLog = timeLogRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("El time log que se intenta pausar no existe en la base de datos"));


        if (timeLog.getEndDateTime() != null) {
            throw new RuntimeException("Se esta intentando pausar un timer que no se ha iniciado");
        }
        timeLog.setEndDateTime(request.getEndDateTime());
    ...
    public List<TimeLogDTO> stop(TimeLogBatchRequest request) {
        ...
        if (request.getId() != null) {
            if (request.getEndDateTime() == null) {
                throw new RuntimeException("Para parar un registro de tiempo activo se debe enviar el momento de finalización.");
            }

            TimeLog activeLog = noCompletedTimeLogs.stream()
                    .filter(log -> log.getId().equals(request.getId()))
                    .findFirst()
                    .orElseThrow(() -> new RuntimeException("El time log que intentas parar no existe o ya estaba completado."));
